...
class unet_att_d(nn.Module):
    def __init__(self, in_channels, filter_config, block_num, dropout_rate=0, 
        ...
        # Expansion Path
        self.decoder_1 = UpconvBlock(filter_config[5], filter_config[4], upmode=upmode)  # 1024x16x16
        ...
        self.decoder_2 = UpconvBlock(filter_config[4], filter_config[3], upmode=upmode)  # 512x32x32
        self.conv2 = ConvBlock(filter_config[3] * 2, filter_config[3], num_conv_layers=block_num[3], 
                               drop_rate=dropout_rate, dropout_type=dropout_type)

        self.decoder_3 = UpconvBlock(filter_config[3], filter_config[2], upmode=upmode)  # 256x64x64
        self.conv3 = ConvBlock(filter_config[2] * 2, filter_config[2], num_conv_layers=block_num[2], 
                               drop_rate=dropout_rate, dropout_type=dropout_type)

        self.decoder_4 = UpconvBlock(filter_config[2], filter_config[1], upmode=upmode)  # 128x128x128
        self.conv4 = ConvBlock(filter_config[1] * 2, filter_config[1], num_conv_layers=block_num[1], 
                               drop_rate=dropout_rate, dropout_type=dropout_type)

        self.decoder_5 = UpconvBlock(filter_config[1], filter_config[0], upmode=upmode)  # 64x256x256
